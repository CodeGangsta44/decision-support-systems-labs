...
public class AntiReflexivityViolationFinder implements BiFunction<FuzzyRelation, FuzzyRelationProperty, FuzzyRelationPropertyViolation> {
    ...
    private List<String> findViolations(final FuzzyRelation fuzzyRelation) {
        ...
        return Stream.concat(firstConstraintMessages(fuzzyRelation), secondConstraintMessages(fuzzyRelation))
                .collect(Collectors.toList());
    }

    private Stream<String> firstConstraintMessages(final FuzzyRelation fuzzyRelation) {

        return IntStream.range(INTEGER_ZERO, fuzzyRelation.getSize())
                .filter(index -> DOUBLE_ZERO != fuzzyRelation.getElement(index, index))
                .mapToObj(index -> buildViolationMessage(index, index, fuzzyRelation));
    }

    private Stream<String> secondConstraintMessages(final FuzzyRelation fuzzyRelation) {

        return IntStream.range(INTEGER_ZERO, fuzzyRelation.getSize())
                .boxed()
                .flatMap(rowIndex -> IntStream.range(INTEGER_ZERO, fuzzyRelation.getSize())
                        .filter(columnIndex -> fuzzyRelation.getElement(rowIndex, columnIndex) == DOUBLE_ONE)
