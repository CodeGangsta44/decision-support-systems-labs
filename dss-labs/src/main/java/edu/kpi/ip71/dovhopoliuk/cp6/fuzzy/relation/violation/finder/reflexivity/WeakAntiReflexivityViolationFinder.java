...
public class WeakAntiReflexivityViolationFinder implements BiFunction<FuzzyRelation, FuzzyRelationProperty, FuzzyRelationPropertyViolation> {
    ...
    private List<String> findViolations(final FuzzyRelation fuzzyRelation) {
        ...
        return IntStream.range(INTEGER_ZERO, fuzzyRelation.getSize())
                ...
                .flatMap(rowIndex -> IntStream.range(INTEGER_ZERO, fuzzyRelation.getSize())
                        .filter(columnIndex -> fuzzyRelation.getElement(rowIndex, columnIndex) >= fuzzyRelation.getElement(rowIndex, rowIndex))
